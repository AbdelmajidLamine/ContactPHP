...
<?php
   ...
   if(isset($_GET['Update'])){
     ...
     if($_SERVER['REQUEST_METHOD']){
        ...
        if(!empty($_POST['nom'])&& !empty($_POST['prenom'])&& !empty($_POST['telephone1'])&& !empty($_POST['telephone2'])&& !empty($_POST['address'])&& !empty($_POST['email1'])&& !empty($_POST['email2'])){
   	    ...
   	    $donnee=$connect->prepare('UPDATE donnee SET nom=:nom,prenom=:prenom,telephone1=:telephone1,telephone2=:telephone2,address=:address,email1=:email1,email2=:email2,image=:image,genre=:ch1 WHERE ID=:ID');
   	    $donnee->execute(array('ID'=>$ID,'nom'=>$nom,'prenom'=>$prenom,'telephone1'=>$telephone1,'telephone2'=>$telephone2,'address'=>$address,'email1'=>$email1,'email2'=>$email2,'image'=>$image,'ch1'=>$ch1));


   	    echo "le contacte a ete bien modifier";
   }
    }
     }
